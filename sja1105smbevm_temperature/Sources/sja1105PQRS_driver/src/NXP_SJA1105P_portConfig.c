...
/* modified for SJA1105SMBEVM gm 2018.02.08 */
/* static */ uint8_t SJA1105P_configPort(uint8_t port, uint8_t switchId, SJA1105P_speed_t speed, SJA1105P_xmiiMode_t xmiiMode, uint8_t phyMode);
...
extern uint8_t SJA1105P_autoConfigPorts(void)
{
	...
	for (switchId = 0; switchId < SJA1105P_N_SWITCHES; switchId++)
	{
		...
		for (port = 0; port < SJA1105P_N_PORTS; port++)
		{
			...
			ret += SJA1105P_configPort(port, switchId, portStatus.speed, portStatus.xmiiMode, portStatus.phyMode);
		}
	}
...
* A value of 0 means disable delay line
...
extern uint8_t SJA1105P_setupClockDelay(uint16_t delay, uint8_t port, uint8_t switchId, SJA1105P_direction_t direction)
{
	...
	lDelay  = (uint16_t) (delay - MINIMUM_CLK_DELAY);    /* remove offset */
	cDelay = (uint8_t)  (lDelay / (STEPSIZE_CLK_DELAY)); /* normalize */
	ret = SJA1105P_getCfgPadMiixId(&cfgPadMiixId, port, switchId);
	if (direction == SJA1105P_e_direction_TX)
	{
		cfgPadMiixId.txcPd = 1;         /* Power Down */
		cfgPadMiixId.txcBypass = 1;     /* No Bypassing */
	...
	{
		cfgPadMiixId.rxcPd = 1;         /* Power Down */
		cfgPadMiixId.rxcBypass = 1;     /* No Bypassing */
		ret += SJA1105P_setCfgPadMiixId(&cfgPadMiixId, port, switchId);
		if (delay > 0) {
